    ...
    return next.handle().pipe(
      tap({
        next: (data) => {
          ...
          // Log metrics
          this.logger.logMetric('WebSocketMessageDuration', durationMs, 'Milliseconds', {
            MessageType: messageType,
          });
          
          // Log message processed
          this.logger.debug(`WebSocket message processed: ${messageType} ${durationMs.toFixed(2)}ms`, {
            messageType,
            connectionId,
        ...
        error: (error) => {
          ...
          // Log error
          this.logger.error(`WebSocket message error: ${messageType} ${error.message}`, {
            messageType,
            connectionId,
            durationMs: durationMs.toFixed(2),
            error: {
