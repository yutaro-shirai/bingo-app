...
describe('PlayerController', () => {
  ...
  describe('registerPlayer', () => {
    it('should register a player successfully', async () => {
      ...
      const result = await controller.registerPlayer({
        gameCode,
        name: playerName,
      });
      ...
      expect(mockPlayerService.generateBingoCard).toHaveBeenCalledWith(
        mockPlayer.id,
      );
  ...
  describe('getPlayerById', () => {
    ...
    it('should throw NotFoundException when player not found', async () => {
      ...
      await expect(controller.getPlayerById(playerId)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updatePunchedNumbers', () => {
    it('should update punched numbers', async () => {
      ...
      const result = await controller.updatePunchedNumbers(playerId, {
        number,
      });

      expect(mockPlayerService.getPlayerById).toHaveBeenCalledWith(playerId);
      expect(mockPlayerService.updateCardState).toHaveBeenCalledWith(playerId, [
        number,
      ]);
  ...
  describe('claimBingo', () => {
    it('should claim bingo for a player', async () => {
      ...
      expect(mockPlayerService.getPlayerById).toHaveBeenCalledWith(playerId);
      expect(mockGameService.getGameById).toHaveBeenCalledWith(
        mockPlayer.gameId,
      );
      expect(mockPlayerService.updateBingoStatus).toHaveBeenCalledWith(
        playerId,
        true,
      );
      expect(result).toEqual(mockUpdatedPlayer);
